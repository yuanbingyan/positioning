...
def distance(lng1, lng2, lat1, lat2):
    ...
    s = s * 6378.137 * 1000
    return s
def polynomial_regression_lat(data,BS_lng,BS_lat,limli_scale,parameter_lng,lte_id):
    ...
    ## 多项式回归结束
    #############计算误差
    print('poly纬度训练误差',np.mean(np.abs(train_y-train_y_pred)))
    ...
    joblib.dump((limli_scale,BS_lng,BS_lat,scaler,poly_reg, lin_reg_2),'polynomial_regression_model/'+lte_id+'_lat.pkl')
